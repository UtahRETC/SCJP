...
public class TestSorted {
	...
	//There are 3 ordered collections on the exam: TreeMap, TreeSet and Priority Queue
	...
	for(Human h : map.keySet()) System.out.println(h.toString());
	set.add(new Human("Fred", "Blue"));
	set.add(new Human("Sally", "Brown"));
	for(Human h :set) System.out.println(h.toString());
	queue.add(new Human("Fred", "Blue"));
	queue.add(new Human("Sally", "Brown"));
	for(Human h : queue) System.out.println(h.toString());
	...
	try {
		...
		for(Ewok e : brokenQ) System.out.println(e.toString());
	...
	try {
		...
		for(Ewok e : brokenS) System.out.println(e.toString());
	} catch (ClassCastException cce) {
		System.out.println(cce);
	}
	try {
		...
		brokenM.put(new Ewok("Wicket", "Brown"), new Human("Jicket", "Brown"));
		for(Ewok e : brokenM.keySet()) System.out.println(e.toString());
	} catch (ClassCastException cce) {
		System.out.println(cce);
	}
	System.out.println("You can only succesfully add objects to sorted collections -- PriorityQueue, TreeMap and TreeSet -- if they implement comparable or have a comparator");
	}
}
...
class Human implements Comparable<Human> {
	...
	Human(String name, String eyeColor) { this.name = name; this.eyeColor = eyeColor; }	
...
class Ewok {
	...
	Ewok(String name, String eyeColor) { this.name = name; this.eyeColor = eyeColor; }
}