...
class Box implements Comparable<Box> {
	...
	Box() { size = (int) (Math.random() * 50); }
	Box(Integer size) { this.size = size; }
	public Integer getSize() { return size; }
	public int compareTo(Box a) { return size.compareTo(a.getSize()); }
	...
	//best practice to override hashCode() every time you override equals, the following is a very, very bad implementation of hashCode()
	public int hashCode() {
		return size;
	}
}
class TestBoxQueue {
	...
	TestBoxQueue() { 
		...
		//the following will only work if you override public boolean equals(Object o) in your custom case
...
class TestIntegerQueue {
	...
	TestIntegerQueue() {
		...
		//Integer is one of the wrapper classes and all the wrapper classes implement natural ordering
		...
		//offer is the same as add except it isn't forced to throw an exception every time it fails, it can 
		//merely return false
		System.out.println(intQueue.offer(i));
		System.out.println(intQueue.contains(i));
		//peek reveals the first element in the queue (which will be the lowest integer in this case, the only integer object)
		...
		intQueue.offer(a); intQueue.offer(b); intQueue.offer(c);
		...
		//these return true because they are identical to the a, b, and c Integer objects
		qu.readableContains(intQueue, d);
		qu.readableContains(intQueue, e);
		qu.readableContains(intQueue, f);

		intQueue.offer(d); intQueue.offer(e); intQueue.offer(f);
		...
		//remember that priority queue only guarantees sorted order when pulling stuff out of the front or end of the queue,
