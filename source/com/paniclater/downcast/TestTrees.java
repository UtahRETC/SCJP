...
public class TestTrees {
	public static void main(String[] args) {
		...
		trees.add(pt);	
		for(FruitTree ft : trees) { ft.makeFruit(); }
		System.out.println(ap.toString());
		System.out.println(pt.toString());
		//when accessing member variables directly polymorphism does not apply -- the value returned is determined by reference variable type
		System.out.println(((Tree) ap).name);
		System.out.println(((FruitTree) ap).name);
		System.out.println(((AppleTree) ap).name);
		//when accessing member variables through METHODS polymorphism does apply -- the value returned is determined by object instance type
