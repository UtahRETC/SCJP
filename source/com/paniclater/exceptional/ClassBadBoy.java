public class ClassBadBoy {
	...
	void safe() {
		try {
			...
			ab = new C();
			//Now that A type reverence var refers to a C object it is totally ok to cast it as a C object.
			C cb = (C) ab;	
			//Same error will be thrown if we try:
			//ab = (C) ab;
			//B IS-A A so our ref variable a can still be assigned to a new B();
			ab = new B();
			//Now we ask the JVM to cast our B object to a C type. The compiler lets this go, because as we saw earlier,
			//it is totally possible for the A reference variable type to be pointing at a C object. However, at runtime we encounter
